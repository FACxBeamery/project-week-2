...
test('String Exists in array - case sensitive', (t) => {
    t.equal(strExistsInArray('fire', ['water', 'fire', 'ground']), true, 'strExistsInArray returns true');
    t.equal(strExistsInArray('2', ['water', '2', 'ground']), true, 'strExistsInArray returns true');
    t.equal(strExistsInArray('aoisndionqsnd', ['water', 'fire', 'aoisndionqsnd']), true, 'strExistsInArray returns true');
    t.equal(strExistsInArray('Firkasmd', ['water', 'Firkasmd', 'ground']), true, 'strExistsInArray returns true');
    t.end();
});

test('String does NOT exist in array', (t) => {
    t.equal(strExistsInArray('fire', ['water', 'Fire', 'ground']), false, 'strExistsInArray returns false');
    t.equal(strExistsInArray('2', ['water', 2, 'ground']), false, 'strExistsInArray returns false');
    t.equal(strExistsInArray('aoisndionqsnd', ['water', 'fire', 'aoisndionqsnD']), false, 'strExistsInArray returns false');
    t.equal(strExistsInArray('Firkasmd', ['water', 'firkasmd', 'ground']), false, 'strExistsInArray returns false');
    t.end();
});
